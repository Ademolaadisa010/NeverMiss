...
type ReminderStatus = "upcoming" | "snoozed" | "dismissed" | "fired";
...
interface Reminder {
  ...
  taskDate: string;
  taskTime: string;
  reminderTime: string; // actual reminder fire time HH:MM
...
const MOCK_REMINDERS: Reminder[] = [
  {
    ...
    taskDescription: "Daily sync with the engineering team",
    taskDate: relativeDate(0),
    taskTime: hhmm(9, 0),
    reminderTime: hhmm(8, 30),
  ...
  },
  {
    ...
    taskDescription: "Leg day — squats and deadlifts",
    taskDate: relativeDate(0),
    taskTime: hhmm(18, 30),
    reminderTime: hhmm(18, 15),
    reminderBefore: "15 min before",
    priority: "low",
    category: "Health",
    status: "snoozed",
    recurring: "Mon/Wed/Fri",
    snoozedUntil: hhmm(18, 25),
  },
  {
    id: "r3",
    taskTitle: "Dentist Appointment",
    taskDescription: "Annual checkup at City Dental Clinic",
    taskDate: relativeDate(1),
    taskTime: hhmm(15, 0),
    reminderTime: hhmm(14, 0),
  ...
  {
    id: "r4",
    taskTitle: "Submit Q3 Report",
    taskDescription: "Q3 performance report for the board",
    taskDate: relativeDate(3),
    taskTime: hhmm(17, 0),
    reminderTime: hhmm(15, 0),
  ...
  },
  {
    ...
    taskDescription: "Monthly utility via bank app",
    taskDate: relativeDate(5),
    taskTime: hhmm(12, 0),
    reminderTime: hhmm(11, 0),
  ...
  },
  {
    ...
    taskDescription: "Morning mindfulness session",
    taskDate: relativeDate(-1),
    taskTime: hhmm(7, 0),
    reminderTime: hhmm(6, 55),
    reminderBefore: "5 min before",
    priority: "low",
    category: "Health",
    status: "dismissed",
    firedAt: "6:55 AM",
  },
  {
    id: "r7",
    taskTitle: "Call Mom",
    taskDescription: "Weekly catch-up",
    taskDate: relativeDate(-2),
    taskTime: hhmm(19, 0),
    reminderTime: hhmm(18, 30),
    reminderBefore: "30 min before",
    priority: "low",
    category: "Personal",
    status: "dismissed",
  ...
  {
    id: "r8",
    taskTitle: "Book Flight Tickets",
    taskDescription: "Return trip to Lagos",
    taskDate: relativeDate(4),
    taskTime: hhmm(10, 0),
    reminderTime: hhmm(9, 0),
    reminderBefore: "1 hour before",
    priority: "medium",
    category: "Personal",
    status: "upcoming",
  ...
  return (
    <div className="rounded-2xl border border-[#f59e0b]/25 bg-gradient-to-r from-[#f59e0b]/10 via-[#f59e0b]/5 to-transparent p-4 flex items-center gap-4 mb-6">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-white/40 leading-relaxed">Get notified even when the app is closed. Never miss a reminder.</p>
      </div>
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          onClick={onEnable}
          className="px-4 py-2 rounded-xl bg-[#f59e0b]/20 border border-[#f59e0b]/30 text-[#f59e0b] text-xs font-bold hover:bg-[#f59e0b]/30 transition-colors"
        >
          Enable
        </button>
        <button
          onClick={onDismiss}
          className="w-7 h-7 rounded-lg border border-white/10 text-white/30 text-xs hover:text-white/60 transition-colors flex items-center justify-center"
        >
          ✕
        </button>
      </div>
...
function SnoozePopover({ onSnooze, onClose }: { onSnooze: (dur: string) => void; onClose: () => void }) {
  return (
    <div className="absolute right-0 top-full mt-2 z-50 w-44 rounded-2xl border border-white/10 bg-[#0f0f24] shadow-2xl shadow-black/40 p-2" onClick={(e) => e.stopPropagation()}>
      <p className="text-[10px] font-bold text-white/30 uppercase tracking-wider px-2 py-1">Snooze for…</p>
      {SNOOZE_OPTIONS.map((opt) => (
        <button
          key={opt}
          onClick={() => { onSnooze(opt); onClose(); }}
...
function ReminderCard({
  ...
  return (
    <div className={`relative group rounded-2xl border transition-all duration-300 ${
      isDismissed
        ? "border-white/5 bg-white/[0.015] opacity-50 hover:opacity-70"
        : isSnoozed
        ? "border-[#60a5fa]/20 bg-[#60a5fa]/5 hover:border-[#60a5fa]/30"
        : `border-white/10 bg-white/[0.04] hover:bg-white/[0.06] hover:border-white/15 shadow-lg ${p.glow}`
    }`}>
      {/* Priority left bar */}
      {isUpcoming && (
        <div className={`absolute left-0 top-5 bottom-5 w-0.5 rounded-full ${p.dot}`} style={{ left: "-1px" }} />
      )}
      ...
      <div className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${
            isDismissed ? "bg-white/5" : isSnoozed ? "bg-[#60a5fa]/15" : "bg-[#a78bfa]/15"
          }`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 mb-1">
              <h3 className={`font-bold text-base leading-snug ${isDismissed ? "text-white/30 line-through" : "text-white"}`}>
                {reminder.taskTitle}
              </h3>
            </div>

            <p className={`text-sm mb-3 ${isDismissed ? "text-white/20" : "text-white/40"}`}>
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs">
              ...
              <span className={`flex items-center gap-1.5 ${isDismissed ? "text-white/20" : "text-[#a78bfa]/70"}`}>
                ...
                Remind at {formatTime(reminder.reminderTime)}
        ...
        {!isDismissed && (
          <div className="flex items-center gap-2 mt-4 pt-4 border-t border-white/5">
            ...
            <div className="relative">
              ...
              {showSnooze && (
                <SnoozePopover
                  onSnooze={(dur) => onSnooze(reminder.id, dur)}
...
function EmptyState({ tab }: { tab: FilterTab }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      {tab !== "dismissed" && (
        <Link href="/create" className="px-5 py-2 rounded-full bg-[#a78bfa]/15 border border-[#a78bfa]/25 text-[#a78bfa] text-xs font-semibold hover:bg-[#a78bfa]/25 transition-colors">
          + Create Task
        </Link>
      )}
    </div>
...
export default function RemindersPage() {
  ...
  const handleSnooze = (id: string, dur: string) => {
    setReminders((prev) =>
      prev.map((r) => r.id === id ? { ...r, status: "snoozed", snoozedUntil: "soon" } : r)
    );
  ...
  const handleDismiss = (id: string) => {
    setReminders((prev) =>
      prev.map((r) => r.id === id ? { ...r, status: "dismissed", firedAt: new Date().toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" }) } : r)
    );
  ...
  const handleRestore = (id: string) => {
    setReminders((prev) =>
      prev.map((r) => r.id === id ? { ...r, status: "upcoming", firedAt: undefined } : r)
    );
  ...
  // Sort: upcoming → snoozed → dismissed
  const statusOrder: Record<ReminderStatus, number> = { upcoming: 0, snoozed: 1, fired: 2, dismissed: 3 };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] font-[var(--font-sora)]">
          ...
          {[
            ...
            <Link key={href} href={href} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${active ? "bg-[#a78bfa]/15 text-[#a78bfa] border border-[#a78bfa]/20" : "text-white/40 hover:text-white/70 hover:bg-white/5"}`}>
              ...
              {active && counts.upcoming > 0 && (
                <span className="ml-auto w-5 h-5 rounded-full bg-[#a78bfa] text-white text-[10px] font-bold flex items-center justify-center">
                  {counts.upcoming}
                </span>
              )}
            </Link>
        ...
        <header className="sticky top-0 z-30 border-b border-white/5 bg-[#0a0a1a]/80 backdrop-blur-md px-6 py-4 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-lg font-bold text-white flex items-center gap-2">
              Reminders
              {counts.upcoming > 0 && (
                <span className="w-5 h-5 rounded-full bg-[#a78bfa] text-white text-[10px] font-bold flex items-center justify-center">
                  {counts.upcoming}
                </span>
              )}
            </h1>
          ...
          <Link
            href="/create"
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-[#a78bfa] to-[#7c3aed] text-white text-sm font-semibold hover:opacity-90 transition-opacity shadow-lg shadow-[#7c3aed]/20"
          >
                ...
                <>
                  {counts.upcoming > 0 && (
                    <section>
                      ...
                      <div className="space-y-3">
                        {sorted.filter((r) => r.status === "upcoming").map((r) => (
                          <ReminderCard key={r.id} reminder={r} onSnooze={handleSnooze} onDismiss={handleDismiss} onRestore={handleRestore} />
                        ))}
                      </div>
                    </section>
                  )}
                  {counts.snoozed > 0 && (
                    <section className="mt-6">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-xs font-bold text-white/30 uppercase tracking-widest">Snoozed</span>
                        <div className="flex-1 h-px bg-white/5" />
                      </div>
                      <div className="space-y-3">
                        {sorted.filter((r) => r.status === "snoozed").map((r) => (
                          <ReminderCard key={r.id} reminder={r} onSnooze={handleSnooze} onDismiss={handleDismiss} onRestore={handleRestore} />
                        ))}
                      </div>
                    </section>
                  ...
                  {counts.dismissed > 0 && (
                    <section className="mt-6">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-xs font-bold text-white/30 uppercase tracking-widest">Dismissed</span>
                        <div className="flex-1 h-px bg-white/5" />
                      </div>
                      <div className="space-y-3">
                        {sorted.filter((r) => r.status === "dismissed").map((r) => (
                          <ReminderCard key={r.id} reminder={r} onSnooze={handleSnooze} onDismiss={handleDismiss} onRestore={handleRestore} />
                        ))}
                      </div>
                    </section>
      ...
      <nav className="fixed bottom-0 inset-x-0 z-40 lg:hidden border-t border-white/5 bg-[#0d0d20]/90 backdrop-blur-md flex items-center justify-around px-4 py-3">
        {[
          ...
          <Link key={href} href={href} className={`flex flex-col items-center gap-1 text-xs font-medium transition-colors relative ${active ? "text-[#a78bfa]" : "text-white/30 hover:text-white/60"}`}>
            ...
            {label}
            {active && counts.upcoming > 0 && (
              <span className="absolute -top-0.5 right-1 w-4 h-4 rounded-full bg-[#a78bfa] text-white text-[9px] font-bold flex items-center justify-center">
                {counts.upcoming}
              </span>
            )}
          </Link>
      ...
      {/* Toast */}
      {toastMsg && (
        <div className="fixed bottom-24 lg:bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-3 rounded-2xl bg-[#1a1a35] border border-white/15 text-white text-sm font-medium shadow-2xl shadow-black/40 backdrop-blur-md whitespace-nowrap animate-fade-in">
          {toastMsg}
        </div>
      )}
    </div>
