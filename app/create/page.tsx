...
// ── Helpers ──────────────────────────────────────────────
function deriveStatus(dateStr: string): Status {
...
function parseAiInput(step: AiStep, input: string): Partial<AiTaskDraft> {
  ...
  switch (step) {
      ...
      if (lower.includes("tomorrow")) {
        ...
        for (const [name, idx] of Object.entries(months)) {
          if (lower.includes(name)) {
            ...
            date = new Date(new Date().getFullYear(), idx, day).toISOString().split("T")[0];
            break;
          }
        }
      }
    ...
    case "recurring": {
      ...
      if (lower.includes("daily") || (lower.includes("day") && !lower.includes("week"))) return { recurring: "Daily" };
...
const inputCls = "w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/25 focus:outline-none focus:border-[#a78bfa]/60 focus:bg-white/8 transition-all";

// ── Form Mode ─────────────────────────────────────────────
function FormMode({ onSave, saving }: { onSave: (data: FormData) => void; saving: boolean }) {
  const [form, setForm] = useState<FormData>({
    title: "", description: "", date: "", time: "",
    priority: "medium", category: "Work",
    reminderBefore: "30 min before", recurring: "None",
  });
    ...
    <div className="space-y-5">
      ...
      <Field label="Priority">
        <div className="flex gap-2">
          {PRIORITY_OPTIONS.map(({ value, label, color, bg }) => (
            <button key={value} onClick={() => set("priority", value)}
              className={`flex-1 py-2.5 rounded-xl border text-sm font-medium transition-all ${form.priority === value ? `${bg} ${color}` : "border-white/8 text-white/30 hover:border-white/20 hover:text-white/50"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {CATEGORIES.map((cat) => (
            <button key={cat} onClick={() => set("category", cat)}
              className={`px-3 py-1.5 rounded-full border text-xs font-medium transition-all ${form.category === cat ? "bg-[#a78bfa]/20 border-[#a78bfa]/40 text-[#a78bfa]" : "border-white/8 text-white/30 hover:border-white/20 hover:text-white/50"}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Remind Me">
          <select className={`${inputCls} [color-scheme:dark]`} value={form.reminderBefore} onChange={(e) => set("reminderBefore", e.target.value)}>
            {REMINDER_OPTIONS.map((o) => <option key={o} value={o}>{o}</option>)}
          </select>
        </Field>
        <Field label="Recurring">
          <select className={`${inputCls} [color-scheme:dark]`} value={form.recurring} onChange={(e) => set("recurring", e.target.value)}>
      ...
      <button
        ...
        disabled={saving}
        className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#a78bfa] to-[#7c3aed] text-white font-bold text-sm hover:opacity-90 active:scale-[0.98] transition-all shadow-lg shadow-[#7c3aed]/25 mt-2 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {saving ? (
...
// ── AI Mode ───────────────────────────────────────────────
function AiMode({ onSave, saving }: { onSave: (data: FormData) => void; saving: boolean }) {
  const [messages, setMessages] = useState<AiMessage[]>([{ role: "ai", text: AI_PROMPTS.task }]);
  ...
  const pushAiMessage = useCallback((text: string) => {
    setIsTyping(true);
    setTimeout(() => { setIsTyping(false); setMessages((p) => [...p, { role: "ai", text }]); }, 900);
  }, []);
  ...
  const handleRestart = () => {
    ...
    setStepIdx(0); setDraft({}); setInput(""); setConfirmed(false);
  };
  ...
  const handleVoice = () => {
    ...
    if (!API) { alert("Voice input isn't supported in your browser. Try Chrome."); return; }
  ...
  const stepLabels: Record<AiStep, string> = { task: "Task", datetime: "Date & Time", recurring: "Recurrence", reminder: "Reminder", confirm: "Confirm" };
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-2">
          {AI_STEPS.map((s, i) => (
            <div key={s} className="flex flex-col items-center gap-1">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all ${i < stepIdx ? "bg-[#a78bfa] text-white" : i === stepIdx ? "bg-[#a78bfa]/30 border-2 border-[#a78bfa] text-[#a78bfa]" : "bg-white/5 border border-white/10 text-white/20"}`}>
                {i < stepIdx ? "✓" : i + 1}
              </div>
              <span className={`text-[10px] font-medium hidden sm:block ${i === stepIdx ? "text-[#a78bfa]" : i < stepIdx ? "text-white/40" : "text-white/15"}`}>{stepLabels[s]}</span>
            </div>
          ))}
        </div>
        <div className="h-0.5 bg-white/5 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-[#a78bfa] to-[#7c3aed] rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
        </div>
      </div>

      {/* Chat */}
      <div className="flex-1 overflow-y-auto space-y-4 py-2 min-h-[280px] max-h-[380px] pr-1">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
            {msg.role === "ai" && (
              <div className="w-8 h-8 rounded-full bg-[#a78bfa]/20 border border-[#a78bfa]/30 flex items-center justify-center text-sm flex-shrink-0">🤖</div>
            )}
            <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${msg.role === "ai" ? "bg-white/5 border border-white/10 text-white/80 rounded-tl-sm" : "bg-gradient-to-br from-[#a78bfa] to-[#7c3aed] text-white rounded-tr-sm"}`}>
              {msg.text}
            </div>
          </div>
        ))}
        ...
        {isTyping && (
          <div className="flex gap-3">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-1.5">
              {[0,1,2].map((i) => <span key={i} className="w-1.5 h-1.5 rounded-full bg-[#a78bfa]/60 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />)}
            </div>
          </div>
        )}
        ...
        {currentStep === "confirm" && !isTyping && !confirmed && (
          <div className="flex gap-3 ml-11">
            <button
              ...
              disabled={saving}
              className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-[#a78bfa] to-[#7c3aed] text-white text-sm font-bold hover:opacity-90 transition-opacity disabled:opacity-60 flex items-center justify-center gap-2"
            >
              {saving ? <><div className="w-3.5 h-3.5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> Saving…</> : "✅ Yes, Save It!"}
            </button>
            <button onClick={handleRestart} className="px-4 py-2.5 rounded-xl border border-white/10 text-white/50 text-sm font-medium hover:border-white/20 hover:text-white/70 transition-all">
              🔄 Restart
            </button>
          </div>
        )}
      ...
      {currentStep !== "confirm" && (
        <div className="mt-4 flex items-center gap-2 border border-white/10 rounded-2xl bg-white/5 p-2 focus-within:border-[#a78bfa]/40 transition-colors">
          <button onClick={handleVoice} className={`w-9 h-9 rounded-xl flex items-center justify-center text-base transition-all flex-shrink-0 ${listening ? "bg-red-500/20 border border-red-500/30 text-red-400 animate-pulse" : "bg-white/5 border border-white/10 text-white/40 hover:text-[#a78bfa] hover:border-[#a78bfa]/30"}`}>
            {listening ? "⏹" : "🎙️"}
          </button>
          <input
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center py-10 px-4">
      <div className="relative w-24 h-24 mb-6">
        ...
        <div className="relative w-24 h-24 rounded-full bg-gradient-to-br from-[#a78bfa] to-[#7c3aed] flex items-center justify-center text-4xl shadow-2xl shadow-[#7c3aed]/40">✅</div>
      </div>
      ...
      <p className="text-white/40 text-sm mb-6 max-w-xs leading-relaxed">
        We'll remind you about <span className="text-[#a78bfa] font-medium">"{task.title}"</span>{" "}
        {task.reminderBefore} on {formatDateDisplay(task.date)} at {formatTime(task.time)}.
      </p>
      <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-left mb-8 space-y-3">
        {[
          ...
          { icon: "📅", label: "Date",      value: `${formatDateDisplay(task.date)} at ${formatTime(task.time)}` },
          ...
          <div key={label} className="flex items-center justify-between gap-4">
            <span className="flex items-center gap-2 text-xs text-white/40"><span>{icon}</span> {label}</span>
            <span className="text-xs font-medium text-white/80 text-right truncate max-w-[160px]">{value}</span>
          </div>
        ))}
      </div>
      <div className="flex gap-3 w-full max-w-sm">
        <button onClick={onAnother} className="flex-1 py-3 rounded-xl border border-white/10 text-white/60 text-sm font-semibold hover:border-white/20 hover:text-white/80 transition-all">
          + Add Another
        </button>
        <button onClick={() => router.push("/dashboard")} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-[#a78bfa] to-[#7c3aed] text-white text-sm font-bold hover:opacity-90 transition-opacity shadow-lg shadow-[#7c3aed]/20">
          Go to Dashboard
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#0a0a1a] font-[var(--font-sora)]">
      ...
      <header className="sticky top-0 z-30 border-b border-white/5 bg-[#0a0a1a]/80 backdrop-blur-md px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm text-white/60">Never<span className="text-[#a78bfa]">Miss</span></span>
        </div>
      </header>
        ...
        {savedTask ? (
          ...
          <>
            <div className="flex items-center gap-1 p-1 rounded-2xl border border-white/8 bg-white/[0.03] mb-6">
              {(["form", "ai"] as Mode[]).map((m) => (
                <button key={m} onClick={() => setMode(m)}
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all ${mode === m ? "bg-gradient-to-r from-[#a78bfa] to-[#7c3aed] text-white shadow-lg shadow-[#7c3aed]/20" : "text-white/35 hover:text-white/60"}`}>
                  {m === "form" ? <><span className="text-base">⌨️</span> Form Mode</> : <><span className="text-base">🤖</span> AI Mode</>}
                </button>
              ))}
            </div>

            <div className={`mb-6 rounded-xl border px-4 py-3 text-xs leading-relaxed ${mode === "ai" ? "border-[#a78bfa]/20 bg-[#a78bfa]/5 text-[#c4b5fd]" : "border-white/8 bg-white/[0.02] text-white/35"}`}>
              {mode === "ai" ? "🎙️ Chat naturally with the AI — type or speak. It'll guide you step-by-step to create your task." : "⚡ Quick and precise. Fill in the fields directly to create your reminder."}
            </div>

            {error && (
              <div className="mb-4 px-4 py-3 rounded-xl border border-red-500/20 bg-red-500/8 text-red-400 text-xs">
                {error}
              </div>
            )}
            ...
            <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-5">
              {mode === "form" ? (
                <FormMode onSave={handleSave} saving={saving} />
              ) : (
                <AiMode key={aiKey} onSave={handleSave} saving={saving} />
              )}
            </div>
          </>
        )}
